...
test('load state from db', async ({page}) => {
  ...
  expect(await page.textContent('[data-testid="last_modified"]')).toContain('Nothing yet')

  await page.type('.ProseMirror', 'foo', {delay})
  expect(await page.textContent('[data-testid="last_modified"]')).not.toContain('Nothing yet')
...
test('open file', async ({page}) => {
  ...
  expect(await page.locator('[data-testid="file_list"] > div').count()).toBe(0)
  ...
  expect(await page.textContent('[data-testid="file_list"] > div:nth-child(1)')).toContain('test2')
  expect(await page.textContent('[data-testid="file_list"] > div:nth-child(2)')).toContain('test1')
